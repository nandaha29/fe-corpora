...
const REGIONS: Region[] = [
  ...
  // 10. MADURA - Pulau kecil atas tengah (#BB65BC)
  {
    ...
    population: "~70K",
    highlights: ["Maritime Culture", "Local Traditions", "Island Heritage"],
    type: "subculture",
    pathData:
...
export function AdvancedPopupMap({ onRegionClick }: AdvancedPopupMapProps) {
  ...
  const [apiRegionData, setApiRegionData] = useState<APIRegionData | null>(null)
  ...
  const fetchRegionData = async (regionId: string) => {
    ...
    try {
      ...
      if (result.success) {
        setApiRegionData(result.data)
      } else {
        throw new Error(result.message || 'Failed to fetch region data')
      }
    } catch (error) {
      console.error('Error fetching region data:', error)
      setRegionError(error instanceof Error ? error.message : 'Unknown error occurred')
    } finally {
              ...
              >
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <h2 className="text-2xl font-bold text-white drop-shadow-lg text-center px-4 line-clamp-2">
                    {apiRegionData ? apiRegionData.name : currentRegion.name}
                  </h2>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-px bg-border/50">
                <div className="bg-card p-3 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-foreground">
                    {apiRegionData ? apiRegionData.lexiconCount : currentRegion.population}
                  </h3>
                  <h3 className="text-lg text-muted-foreground">
                    {apiRegionData ? 'Lexicons' : 'Population'}
                  </h3>
                </div>
                <div className="bg-card p-3 text-center">
                ...
                <div className="bg-card p-3 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-foreground">
                    {apiRegionData ? apiRegionData.subcultureCount : '4.8'}
                  </h3>
                  <h3 className="text-lg text-muted-foreground">
                    {apiRegionData ? 'Subcultures' : 'Rating'}
                  </h3>
                </div>
              </div>
              ...
              <div className="p-4 space-y-3 max-h-[300px] overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent">
                {isLoadingRegion ? (
                  <div className="flex items-center justify-center py-8">
                    ...
                    <span className="ml-2 text-lg text-muted-foreground">Loading region data...</span>
                  </div>
                ) : regionError ? (
                  <div className="text-center py-8">
                    <p className="text-lg text-red-500 mb-2">Failed to load region data</p>
                    <p className="text-sm text-muted-foreground">{regionError}</p>
                  </div>
                ) : (
                  <>
                    <h3 className="text-lg text-muted-foreground leading-relaxed">
                      {apiRegionData ? apiRegionData.description : currentRegion.description}
                    </h3>
